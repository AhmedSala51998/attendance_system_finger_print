...
if (isset($_POST['save_holiday'])) {
    ...
    if ($id > 0) {
        ...
        if ($stmt->execute()) {
            ...
            $msg = "تم تحديث الإجازة بنجاح!";

            // 🔔 إرسال إشعار لكل الموظفين
            $employees = $conn->query("SELECT id FROM employees WHERE role='employee'");

            $title = "تحديث إجازة رسمية ⚠️";
            $notification_desc = "تم تعديل الإجازة إلى: $desc بتاريخ $date";
            ...
            while($emp = $employees->fetch_assoc()){
                $stmt2->bind_param("iss", $emp['id'], $title, $notification_desc);
                $stmt2->execute();
            }
        }
    } else {
        ...
        if ($stmt->execute()) {
            ...
            $title = "إجازة رسمية جديدة 🎉";
            $notification_desc = "تم إضافة إجازة رسمية بتاريخ $date: $desc";
