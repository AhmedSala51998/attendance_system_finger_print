...
// منع تسجيل الحضور قبل الشيفت بوقت كبير
$now = new DateTime();
$shift_start = new DateTime(date('Y-m-d ' . $shift['start_time']));
$allowed_checkin_time = (clone $shift_start)->modify('-30 minutes');

if($now < $allowed_checkin_time){
    echo json_encode([
        'status' => 'error',
        'message' => 'لا يمكن تسجيل الحضور الآن. يمكنك الحضور قبل الشيفت بنصف ساعة فقط.'
    ]);
    exit();
}
...
if($perm){
    $perm_from = new DateTime($perm['from_time']);
    $perm_to   = new DateTime($perm['to_time']);

    if($now >= $perm_from && $now <= $perm_to){
        $has_permission = true;
    }
}

if($late_minutes > 15 && !$has_permission){
    $status = 'late';
}else{
    $status = 'present';
}
